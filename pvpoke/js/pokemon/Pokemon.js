...
function Pokemon(data, i, b){
	...
	for(var i = 0; i < data.fastMoves.length; i++){
		var move = GameMaster.getMoveById(data.fastMoves[i]);
	...
	for(var i = 0; i < data.chargedMoves.length; i++){
		var move = GameMaster.getMoveById(data.chargedMoves[i]);
	...
	if((data.tags)&&(data.tags.indexOf("shadoweligible") > -1)){
		...
		if(data.level25CP <= b.getCP()){
			self.chargedMovePool.push(GameMaster.getMoveById("RETURN"));
	...
	if((data.tags)&&(data.tags.indexOf("shadow") > -1)){
		...
		if(data.tags.indexOf("shadow") > -1){
			self.chargedMovePool.push(GameMaster.getMoveById("FRUSTRATION"));
		}
	...
	this.initializeMove = function(move){
		...
		if(move.energy > 0){
			...
			if(move.buffs){
				...
				if(buffEffect > 0){
					multiplier = ( (GameMaster.data.settings.buffDivisor +(buffEffect* move.buffApplyChance)) / GameMaster.data.settings.buffDivisor);
				}
	...
	this.generateMoveUsage = function(opponent, weightModifier){
		...
		var yawn = GameMaster.getMoveById("YAWN");
		yawn.damage = 1;
	...
	this.addNewMove = function(id, movepool, selectNewMove, moveType, index){
		var move = GameMaster.getMoveById(id);
		...
		// GameMaster.modifyPokemonEntry(self.speciesId, "movepool", props);
	...
	this.generateTraits = function(){
		...
		if(GameMaster.rankings[key]){
			rankings = GameMaster.rankings[key];
	...
	this.generateSimilarPokemon = function(traits){
		...
		var pokemonList = GameMaster.generateFilteredPokemonList(battle, battle.getCup().include, battle.getCup().exclude);
	...
	*/
	...
	function getTypeEffectivenessArray(battle){
		...
		for(var n = 0; n < allTypes.length; n++){
			let effectiveness = battle.getEffectiveness(allTypes[n], self.types);
	...
	this.applyStatBuffs = function(buffs){
		...
		var maxBuffStages = GameMaster.data.settings.maxBuffStages;
	...
	this.getEffectiveStat = function(index, useStartStatBuffs){
		...
		var buffDivisor = GameMaster.data.settings.buffDivisor;
	...
	this.generateURLPokeStr = function(context){
		...
		if((self.isCustom)||(self.startStatBuffs[0] != 0)||(self.startStatBuffs[1] != 0)){
			...
			arr.push(self.ivs.atk, self.ivs.def, self.ivs.hp, self.startStatBuffs[0]+GameMaster.data.settings.maxBuffStages, self.startStatBuffs[1]+GameMaster.data.settings.maxBuffStages, self.baitShields, self.optimizeMoveTiming ? 1 : 0);
	...
	this.setShadowType = function(val){
		...
		if(self.shadowType == "shadow"){
			self.shadowAtkMult = GameMaster.data.settings.shadowAtkMult;
			self.shadowDefMult = GameMaster.data.settings.shadowDefMult;
