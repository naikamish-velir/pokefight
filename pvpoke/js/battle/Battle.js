...
import TimelineAction from './TimelineAction.js'
...
function Battle(){
	...
	self.setNewPokemon = function(poke, index, initialize){
		initialize = typeof initialize !== 'undefined' ? initialize : true;

		let selectedPokemon=new Pokemon(poke,index,this);
		selectedPokemon.initialize(this.getCP(),"gamemaster");
		selectedPokemon.selectRecommendedMoveset();
		selectedPokemon.setShields(0);
		selectedPokemon.autoLevel=true;

		selectedPokemon.setBattle(self);

		if(initialize){
			selectedPokemon.initialize(cp);
		}

		selectedPokemon.index = index;
		pokemon[index] = selectedPokemon;

		// Set shields for corresponding player

		if(mode == "emulate"){

			selectedPokemon.shields = players[index].getShields();
			selectedPokemon.startingShields = players[index].getShields();
			selectedPokemon.priority = players[index].getPriority();
	...
	self.clearPokemon = function(){
		pokemon = [null, null];
	}

	// Set the active players

	self.setPlayers = function(arr){
		players = arr;
	...
	self.getLevelCap = function(){
		return levelCap;
	}

	// Set cup object from Game Master

	self.setCup = function(cupName){
		cup = GameMaster.getCupById(cupName);
	...
	self.calculateDamageByStats = function(attacker, defender, attack, defense, effectiveness, move){
	...
	self.calculateBreakpoint = function(attacker, defender, damage, defense, effectiveness, move){
	...
	self.calculateBulkpoint = function(attacker, defender, damage, attack, effectiveness, move){
	...
	self.getEffectiveness = function(moveType, targetTypes){
		var effectiveness = 1;
		...
		for(var i = 0; i < targetTypes.length; i++){
			...
			var traits = self.getTypeTraits(type);
	...
	self.getTypeTraits = function(type){
		var traits = {
			weaknesses: [],
			resistances: [],
			immunities: []
	...
	self.step = function(){
		...
		if(turns > lastProcessedTurn){
			for(var i = 0; i < 2; i++){
				...
				var poke = pokemon[i];
				var opponent = self.getOpponent(i);
	...
	self.emulate = function(callback){
		mode = "emulate";
		sandbox = true;
	...
	self.getTurnAction = function(poke, opponent){
		var action = null;

		// If Pokemon can take action

		let chargedMoveUsed = false; // Flag so Pokemon only uses one Charged Move per round
	...
	self.decideAction = function(poke, opponent){

		let chargedMoveReady = [];
		let winsCMP = poke.stats.atk >= opponent.stats.atk;

		let fastDamage = self.calculateDamage(poke, opponent, poke.fastMove);
...
//					self.logDecision(turns, poke, " wants to use " + poke.chargedMoves[n].name + " because it has the energy for it. Opponent hp will be " + newOppHealth + ". Turn = " + (currState.turn));
	...
	self.decideRandomAction = function(poke, opponent){
	...
	self.chooseOption = function(options){
		var optionBucket = [];
	...
	self.queueAction = function(actor, type, value){
	...
	self.processAction = function(action, poke, opponent){
	...
	self.useMove = function(attacker, defender, move, forceShields, forceBuff, charge){
		charge = typeof charge !== 'undefined' ? charge : 1;
	...
	self.wouldShield = function(attacker, defender, move){
		var useShield = false;
		var shieldWeight = 1;
	...
	self.getBattleRatings = function(){
		return battleRatings;
	}

	// Return turns to win

	self.getTurnsToWin = function(){
		return turnsToWin;
	}

	// Reset the current turn number

	self.setTurns = function(val){
		turns = val;
	}
	...
	self.getRatingColor = function(rating){
		var winColors = [
			[93,71,165],
			[0,143,187]
		]; // rgb
	...
	self.convertTimelineToActions = function(){
		var actions = [];
	...
	self.calculateTurnMargin = function(){
		var turnMargin = 0;
	...
	self.pushAnimation = function(actor, type, value){
		turnAnimations.push({
			actor: actor,
			type: type,
			value: value
	...
	self.setBattleMode = function(val){
		mode = val;
	}

	// Set whether or not the simulator will follow user input

	self.setSandboxMode = function(val){
		sandbox = val;
	...
	self.logDecision = function(turn, pokemon, string){
		decisionLog.push({
			turn: turn,
			pokemon: pokemon,
			hp: pokemon.hp,
